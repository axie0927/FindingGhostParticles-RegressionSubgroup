...
def plot_results(y_test, y_pred, best_params, save_dir="plots/"):
    ...
    plt.title("Predicted vs Actual Values (RF)")
    plt.savefig(os.path.join(save_dir, 'rf_predicted_vs_actual.png'), dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.savefig(os.path.join(save_dir, 'rf_predicted_vs_test_distribution.png'), dpi=300)
    plt.close()
...
def main():
    """Main function to run the RF Model."""
    cwd = os.getcwd()
    ...
    rf_model, best_params = train_rf_model(X_train_standardized, y_train)

    print(f"Best Hyperparameters: {best_params}")

    print("Evaluating model...")
    mse, mae, r2, y_test, y_pred = evaluate_model(rf_model, X_test_standardized, y_test)
    ...
    print('------------------------------------------------------------------------------------------')
    print("Although this RF model has a relatively lower MSE on the test set, but its runtime, variance explained and MSE are all worse than NN.")
    print('Thus we will not use it on the final NPML dataset.')
